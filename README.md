...
> 基于 Spring AI + Qdrant + Ollama 构建的企业咨询知识库平台
...
**知行信咨询助手** 是一个面向企业管理咨询场景的智能知识库系统。它结合了大语言模型（LLM）和向量检索技术（RAG），帮助企业高效管理咨询资料、快速检索信息，并基于文档内容提供智能问答服务，同时保证不同客户之间的资料严格隔离。

**核心价值**：
- 📄 **文档智能管理**：支持 PDF、Word、TXT 等多格式文档上传与向量化存储
- 🔍 **RAG 智能问答**：基于文档内容的精准问答，支持流式输出
- 🔐 **企业级权限控制**：按客户公司隔离的文档可见性管理
...
- **双因素登录**：支持密码登录 + 邮箱验证码
- **基于角色的访问控制（RBAC）**：内部管理员 / 客户管理员 / 内部员工 / 客户员工
- **开户由我们统一办理**：不开放自助注册（`/register` 是「联系我们开通」说明页）；
  开户路径 = 公司管理页新建公司 → 批量导入员工 → 首次登录强制改密
- **批量导入支持下载模板**：模板由服务端生成（表头与解析器共用同一组列名，
  不可能对不上），填好再上传；导入逐行给结果，哪一行有问题会指到具体行号

### 1️⃣-bis 公司管理与套餐
- **公司管理页**：列表（员工数 / 文档数 / 近 30 天提问数）+ 新建/编辑/停用恢复
- **详情下钻**：合约与状态、员工名单（可指定客户管理员）、专属文档、用量趋势
- **合约字段**：服务到期日、席位上限（可启用账号数）、内部备注
- **套餐硬拦**：停用或到期 → 该公司员工**立即**无法登录（资料保留，续期后自动恢复）；
  席位超限 → 导入时逐行提示，不整批失败
- **到期提醒**：提前 30 / 7 天 —— 内部收汇总邮件、客户登录看到顶栏横幅、
  公司列表标「即将到期 · N 天」；同一档只发一次（发失败次日重试）

### 2️⃣ 客户公司与数据隔离
- 按 `client_id` 隔离：客户只能看**本公司专属文档 + 通用文档**
- 通用文档（咨询公司自己的方法论、资料）对所有客户可见
- 内部顾问可见所有客户的内容
- 权限作用范围由 `user_type` 区分：`1`=内部、`2`=外部客户

### 3️⃣ 文档管理
- 本地文件上传（PDF、DOC、DOCX、TXT、MD）
- URL 远程导入（支持 AWS S3 预签名 URL）
- 文档自动解析与向量化
- 文档归属两档：**本公司专属 / 通用公开**（靠 `client_id` 判定）
- 文档删除（联动删除向量数据）

### 4️⃣ RAG 智能问答
- 基于文档内容的精准问答
- 流式输出（实时逐字显示）
- 客户隔离过滤（客户仅可见本公司 + 通用文档）
- 本地知识库（常见问题秒级响应）
